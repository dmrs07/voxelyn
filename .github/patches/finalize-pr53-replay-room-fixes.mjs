...
root.scripts['build:survival'] = root.scripts['build:survival'].replace(
  'node .github/patches/apply-pr53-replay-room-fixes.mjs && ',
  '',
);
...
client.scripts['build:offline-check'] = client.scripts['build:offline-check'].replace(
  ' && node ../../.github/patches/finalize-pr53-replay-room-fixes.mjs',
  '',
);
writeFileSync(clientPath, `${JSON.stringify(client, null, 2)}\n`);

for (const path of [
  '.github/patches/apply-pr53-replay-room-fixes.mjs',
  '.github/patches/finalize-pr53-replay-room-fixes.mjs',
]) {
...
execFileSync('git', ['commit', '-m', 'fix(survival): canonicalize replays and retain terminal rooms'], { stdio: 'inherit' });
