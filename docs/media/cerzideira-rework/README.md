...
- **A rede (abaixo de 20%).** Ela carrega a rede com aviso longo (26 ticks, faixa no chão num dos oito rumos autorados) e arremessa um disco de teia a 9 tiles/s, alcance 12. Não fere: quem é acertado fica 3 s encapsulado, imune e parado, e ao sair anda a 10% por 4 s, cheio de fios. Recarga de 12 s. O disco é o atlas `fx-silk-net`, oito rumos, três quadros de giro, côncavo como a teia do Homem-Aranha: a borda vai na frente e o centro fica para trás, uma cúpula aberta para o alvo. A Cerzideira veste vermelho e azul (só a rainha; Costureiros e crias seguem na seda mineral): a paleta mestra ganhou `crimson`, `cobalt` e `navy`, os dois azuis porque o único azul saturado que existia, `electric`, é emissivo e viraria halo num corpo. O casulo é o atlas `fx-silk-cocoon` (96×112, sob demanda no grupo da Cerzideira), carimbado sobre o Prospector pelo relógio do estado: a seda sobe do chão em 0,4 s (`attack`), o ovo respira com faixas em hélice, nós de resina e buracos que mostram o escuro (`idle`), racha em duas metades na soltura (`burst`) e deixa tufos e fios pendentes no corpo (`loose`). O parceiro do co-op vê o mesmo quadro (`cocoonUntil`/`webbedUntil` viajam no snapshot e entram no hash).
- **Fim.** A morte da Cerzideira encerra os auxiliares e dissolve a teia.

### A câmara habitada

- **Props de seda** (decoração derivada da seed, sem hash nem snapshot, só em setores com a ocupação dos Costureiros): teia de canto e casulo que se mexe (três quadros de espasmo, o único decor animado) e casa de aranha em domo nas paredes; teia de chão em funil e ninhada de ovos pelo chão; fios com um casulinho pendurados do teto. Nove quadros, encaixados nas vagas da última linha do atlas de props — uma linha nova custaria 2,2 MiB de boot que o orçamento não tem, por isso só a teia de chão tem duas variantes.
- **Aranhinhas** (`silk_spiderling`, fauna autoritativa): dez por setor, nascem em volta das suturas, nunca a menos de 8 tiles da entrada. Inofensivas (dano de contato zero, fora do abate e do Codex), passeiam em passinhos curtos e **fogem ao avistar** um Prospector a até 6 tiles com linha de visão, por 1,8 s renovados enquanto ele estiver à vista, deslizando na parede quando o caminho reto fecha. Um passo em cima as esmaga. Atlas próprio de 48×40 em quatro rumos, sob demanda com o grupo da Cerzideira.
...
O validador de conteúdo manteve os limites existentes: 159,52 MiB no boot e 46,31 MiB sob demanda, abaixo dos tetos de 160 e 48 MiB. Versões atuais: protocolo 39, simulação 76, conteúdo 40. Cliente e servidor precisam ser atualizados juntos.
