...
export class GameRoom {
  ...
  private entitySnapshots(): EntitySnapshot[] {
    ...
    for (let i = 0; i < this.state.players.length; i++) {
      ...
      out.push({
        ...
        freeze: this.state.playerExtras[i].freeze,
  ...
  private viewerState(slot: number): ViewerState {
    ...
    return {
      ...
      cocoonUntil: e.cocoonUntil,
      webbedUntil: e.webbedUntil,
    };
  }
  ...
  private worldFlags(): WorldFlags {
    return {
      ...
      leylineCircuit: {
        closed: this.state.leylineCircuit.closed,
        live: this.state.leylineCircuit.live,
        lit: this.state.leylineCircuit.reached.length,
        total: this.state.leylineCircuit.members.length,
      },
    };
  }
